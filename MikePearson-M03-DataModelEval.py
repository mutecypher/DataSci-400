...
# -*- coding: utf-8 -*-
"""
Created on Monday October 1 19:05:27 2018
...
print ("\nFor the K Nearest Neighbor - TP, TN, FP, FN:", tp, ",", tn, ",", fp, ",", fn)
...
print ("\nArea Under the Curve score for K Nearest Neighbor (using auc function):", np.round(AUC, 2))
print ("\nAUC score for K Nearest Neighbor(using roc_auc_score function):", np.round(roc_auc_score(target_array, prob_out[:,1]), 2), "\n")


## After using the K Nearest Neighbor classifier I
## tried a few others. The Random Forest Classifier worked the next best



# Random Forest classifier
estimators = 6 # number of trees parameter
mss = 2 # mininum samples split parameter
print ('\n\nRandom Forest classifier\n')
clf = RandomForestClassifier(n_estimators=estimators, min_samples_split=mss) # default parameters are fine
clf.fit(train_features, train_targets)

## Get the probability predictions to use for the Area Under The Curve and ROC


prob_2_out = clf.predict_proba(test_features)
print ("predictions for test set:")
bob = clf.predict(test_features)
print ('actual class values:')
print (sum(bob!= target_array))


CMB = confusion_matrix(target_array, bob)
print ("\n\nConfusion matrix:\n", CM)
tnRF, fpRF, fnRF, tpRF = CM.ravel()
print ("\nFor the Random Forest - TP, TN, FP, FN:", tpRF, ",", tnRF, ",", fpRF, ",", fnRF)
ARB = accuracy_score(target_array, bob)
print ("\nAccuracy rate:", ARB)
ERB = 1.0 - AR
print ("\nError rate:", ERB)
PB = precision_score(target_array, bob)
print ("\nPrecision:", np.round(PB, 2))
RB = recall_score(target_array, bob)
print ("\nRecall:", np.round(RB, 2))
F1B = f1_score(target_array, bob)
print ("\nF1 score:", np.round(F1B, 2))
####################
...
## here's where I use the output from the clf.predict_proba for probabilities
## of each prediction
fpr, tpr, th = roc_curve(target_array, prob_2_out[:,1]) # False Positive Rate, True Posisive Rate, probability thresholds
AUC2 = auc(fpr, tpr)
print ("\nTP rates:", np.round(tpr, 2))
print ("\nFP rates:", np.round(fpr, 2))
print ("\nProbability thresholds:", np.round(th, 2))
...
plt.xlabel('FALSE Positive Rate')
plt.ylabel('TRUE Positive Rate')
plt.plot(fpr, tpr, color=LC,lw=LW, label='ROC curve (area = %0.2f)' % AUC2)
plt.plot([0, 1], [0, 1], color='navy', lw=LW, linestyle='--') # reference line for random classifier
plt.legend(loc=LL)
plt.show()
####################
...
print ("\nArea Under the Curve score for Random Forest (using auc function):", np.round(AUC2, 2))
print ("\nArea Under the Curve for Random Forest -  score (using roc_auc_score function):", np.round(roc_auc_score(target_array, prob_2_out[:,1]), 2), "\n")
...
## I used the probability predicted output to create an ROC plot and calculate the 
## Area under the Curve
## Then I used the Randome Forest classifier to obtain similar data
## and I found the Accuracy Rate, the Error Rate, the Precision and Recall
## I also found the Confusion Matrix
## And finally I plotted the ROC, getting  an Area Under the Curve score
## The K-nearest neighbor had an area under the curve that was in the .88 to .91 range
## Depending upon how the randomization worked
## The Random Forest had an Area Under The Curve of about 0.81
